...
def get_order(arr,nmax):
    """
    Vectorized Q-tensor order parameter:
    Q_ab = (1/(2N)) * sum_{ij} (3 l_a l_b - delta_ab)
    returns max eigenvalue of Q.
    """
    delta = np.eye(3)
    # lab shape: (3, nmax, nmax)
    lab = np.vstack((np.cos(arr), np.sin(arr), np.zeros_like(arr))).reshape(3, nmax, nmax)
    # sum over i,j: result (3,3)
    S = np.einsum('aij,bij->ab', lab, lab)  # sum l_a l_b over lattice
    Qab = (3.0 * S - (nmax*nmax) * delta) / (2.0 * nmax * nmax)
